...
router.get("/", async (req, res) => {
  try {
    ...
    if (specificDate && /^\d{4}-\d{2}-\d{2}$/.test(specificDate)) {
      ...
      if (serviceTypeIds?.length) {
        ...
        const sql = `
          ...
          FROM orders
          WHERE store_id = $1
            AND status IN ('topshirildi', 'ready')
        ...
        const summaryRes = await pool.query(`
          ...
          WHERE o.store_id = $1 AND o.status IN ('topshirildi', 'ready') ${svcFilter2} ${dateFilter2}
        `, queryParams);
        ...
        const sql = `
          SELECT
            ...
            unit
          FROM orders
          WHERE store_id = $1 AND status IN ('topshirildi', 'ready') AND (created_at + interval '5 hours')::date = $2::date
          GROUP BY period, service_type_id, service_type_name, unit
          ORDER BY period DESC, service_type_name
        `;
        const summaryRes = await pool.query(`
          ...
          WHERE o.store_id = $1 AND o.status IN ('topshirildi', 'ready') ${dateFilter2}
    ...
    const summaryRes = await pool.query(`
      ...
      WHERE o.store_id = $1
        AND o.status IN ('topshirildi', 'ready')
...
router.get("/orders", async (req, res) => {
  try {
    ...
    const oConditions: string[] = ["o.store_id = $1", "o.status IN ('topshirildi', 'ready')"];
